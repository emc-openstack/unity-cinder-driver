...
class CommonAdapter(object):
    ...
    def retype(self, ctxt, volume, new_type, diff, host):
        ...
        LOG.info('Retype Volume: %(volume_name)s, volume host: '
                 '%(volume_host)s, new type: %(new_type)s, host: %(host)s.',
                 {'volume_name': volume.name, 'volume_host': volume['host'],
                  'new_type': new_type, 'host': host['host']})
        ...
        if need_change_qos:
            new_qos_dict = utils.get_values_from_qos_specs(
                new_qos_specs.get(utils.QOS_SPECS))
            LOG.debug('Driver needs to change QoS, qos: $(new_qos_dict)s.',
                      {'new_qos_dict': new_qos_dict})
            ...
            if new_io_policy:
                ...
                old_qos_dict = utils.get_values_from_qos_specs(
                    old_qos_specs.get(utils.QOS_SPECS))
        ...
        if need_change_compress[0]:
            LOG.debug('Driver needs to change compression.')
